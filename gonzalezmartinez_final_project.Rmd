...
Then created a graph that would allow highlights, which required some changes to the graph structure previously used. In this graph
...
I like this plot because by default it shows the national trend and people can then make comparisons between regions by playing with the Double click features in the legend. **You should give it a try!**

I wanted to introduce a unified hovering line to show values from different regions simultaneously, but thisis where I found the limit to my coding abilities. More about this graph idea in the appendix
